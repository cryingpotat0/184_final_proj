...
Ray Camera::generate_ray(double x, double y) const {
  ...
  Vector3D bottomLeft(-tan(radians(hFov)*.5), -tan(radians(vFov)*.5),-1);
  Vector3D topRight( tan(radians(hFov)*.5),  tan(radians(vFov)*.5),-1);
  auto curr = bottomLeft + Vector3D(x*(topRight.x - bottomLeft.x), y*(topRight.y - bottomLeft.y), 0);
  curr = c2w * curr;
  curr /= curr.norm();
  Ray ret(pos, curr);
  ret.max_t = fClip;
