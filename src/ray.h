...
struct Ray {
  ...
  const float step_size = 0.01;//0.01;
  ...
  Ray surface_tangent() const {
     ...
     return Ray(o, (d + Vector3D(5 * sin(o.x), 5 * sin(o.y), sin(o.z))).unit());
  }

  Ray next_ray() const {
     auto tangent = surface_tangent();
     auto new_orig = tangent.at_time(tangent.step_size);
     //reproject onto surface of sphere
  ...
  Ray& operator=(Ray const&) { return *this; }
};
