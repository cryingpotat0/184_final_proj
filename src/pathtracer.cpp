...
Spectrum PathTracer::estimate_direct_lighting_hemisphere(const Ray& r, const Intersection& isect) {
  ...
  for (int i=0; i < num_samples; i++) {
    auto wi = hemisphereSampler->get_sample();
    auto wi_world = o2w * wi;
    //wi_world /= wi_world.norm();
    auto ray = Ray(hit_p + EPS_D * wi_world, wi_world);
    Intersection intersection;
    if (bvh->intersect(ray, &intersection)) {
      auto val = intersection.bsdf->get_emission() * isect.bsdf->f(w_out, wi) * wi.z / pdf;
      L_out += val;
    }
  }
...
Spectrum PathTracer::estimate_direct_lighting_importance(const Ray& r, const Intersection& isect) {
  ...
  // COMMENT OUT `normal_shading` IN `est_radiance_global_illumination` BEFORE YOU BEGIN
  size_t curr_samples = ns_area_light;

  for(auto light : scene->lights) {
    //if (light->is_delta_light()) curr_samples = 1;
    //else curr_samples = ns_area_light;
...
Spectrum PathTracer::at_least_one_bounce_radiance(const Ray&r, const Intersection& isect) {
  ...
  if (r.depth <= 1) {
    ...
    Vector3D w_in;
    float pdf;
    auto radiance = isect.bsdf->sample_f(w_out, &w_in, &pdf);
    auto w_in_world = o2w * w_in;
    auto new_ray = Ray(hit_p + EPS_D * w_in_world, w_in_world);
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  int actual_samples_taken = 0;
  Vector2D origin = Vector2D(x,y);    // bottom left corner of the pixel
  Spectrum s(0,0,0);
  if (num_samples == 1) {
    auto curr = origin + Vector2D(0.5, 0.5);
    Ray r = camera->generate_ray(
            curr.x / (double) sampleBuffer.w,
            curr.y / (double) sampleBuffer.h
    );
    ...
    for (int i = 0; i < num_samples; i++) {
      ...
      Ray r = camera->generate_ray(
              ((sample.x + origin.x)) / (double) sampleBuffer.w,
      ...
      sum += new_spec.illum();
      square_sum += (new_spec.illum()) * new_spec.illum();
      if (actual_samples_taken % samplesPerBatch == 0) {
        auto mean = sum / actual_samples_taken;
        auto var = 1 / ((double) (actual_samples_taken - 1)) * (square_sum - sum * sum / (double) actual_samples_taken);
        auto I = 1.96 * sqrt(var / actual_samples_taken);
        if (I <= maxTolerance * mean) {
          break;
        }
      }
    }
    s /= (double) actual_samples_taken;
  }

  sampleCountBuffer[x + y * frameBuffer.w] = actual_samples_taken;
  return s;
